...
(7,'MP4 SD','ffmpeg -i {$pathFileName} -vf scale=720:540 -movflags faststart -preset ultrafast -vcodec h264 -acodec aac  -y {$destinationFile}',now(),now(),'mp4','mp4',11),
(8,'MP4 HD','ffmpeg -i {$pathFileName} -vf scale=1280:720 -movflags faststart -preset ultrafast -vcodec h264 -acodec aac  -y {$destinationFile}',now(),now(),'mp4','mp4',12),
