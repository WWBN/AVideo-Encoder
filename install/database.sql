...
-- preserve aspect ratio https://stackoverflow.com/questions/32463531/ffmpeg-resize-largest-video-dimension-to-320
INSERT INTO `formats` VALUES 
(1,'MP4 Low','ffmpeg -i {$pathFileName} -vf \'scale=360:360:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\' -movflags faststart -preset ultrafast -vcodec h264 -acodec aac -strict -2 -y {$destinationFile}',now(),now(),'mp4','mp4',10),
(2,'WEBM Low','ffmpeg -i {$pathFileName} -vf \'scale=360:360:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\'  -movflags faststart -preset ultrafast -f webm -c:v libvpx -b:v 1M -acodec libvorbis -y {$destinationFile}',now(),now(),'webm','mp4',20),
...
(7,'MP4 SD','ffmpeg -i {$pathFileName} -vf \'scale=720:720:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\'  -movflags faststart -preset ultrafast -vcodec h264 -acodec aac -strict -2 -y {$destinationFile}',now(),now(),'mp4','mp4',11),
(8,'MP4 HD','ffmpeg -i {$pathFileName} -vf \'scale=1280:1280:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\'  -movflags faststart -preset ultrafast -vcodec h264 -acodec aac -strict -2 -y {$destinationFile}',now(),now(),'mp4','mp4',12),
(9,'WEBM SD','ffmpeg -i {$pathFileName} -vf \'scale=720:720:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\'  -movflags faststart -preset ultrafast -f webm -c:v libvpx -b:v 1M -acodec libvorbis -y {$destinationFile}',now(),now(),'webm','mp4',21),
(10,'WEBM HD','ffmpeg -i {$pathFileName} -vf \'scale=1280:1280:force_original_aspect_ratio=decrease, pad=ceil(iw/2)*2:ceil(ih/2)*2\'  -movflags faststart -preset ultrafast -f webm -c:v libvpx -b:v 1M -acodec libvorbis -y {$destinationFile}',now(),now(),'webm','mp4',22),
