...
try {
    ...
    if (PHP_SAPI !== 'cli') { $stage = 'streamer'; installerValidateStreamer($data); }
    $stage = 'configuration';
    ...
    $steps[] = ['label' => 'Streamer account and Encoder settings created'];
    $stage = 'configuration';
    ...
    echo json_encode($response = ['error' => false, 'success' => true, 'installed' => true, 'msg' => 'Installation complete. Open the Encoder to sign in.', 'url' => $data['webSiteRootURL'], 'steps' => $steps]);
} catch (Throwable $e) {
    if ($transaction) {
        try { $mysqli->rollback(); }
