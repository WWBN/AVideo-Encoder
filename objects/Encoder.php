...
//$obj->progress = empty($m[1]) ? 0 : intval($m[1]);
    ...
    static function getGifImage($pathFileName, $seconds = 5, $howLong=3) {
        ...
        //Generate a palette:
        eval('$ffmpeg ="ffmpeg -y -ss {$duration} -t {$howLong} -i {$pathFileName} -vf fps=10,scale=160:-1:flags=lanczos,palettegen {$pathFileName}palette.png";');
        exec($ffmpeg . " < /dev/null 2>&1", $output, $return_val);
        if ($return_val !== 0) {
            error_log("Create Pallete Gif Image error: {$ffmpeg}");
            return $global['systemRootPath'] . "view/img/notfound.gif";
        } else {
            eval('$ffmpeg ="ffmpeg -ss {$duration} -t {$howLong} -i {$pathFileName} -i {$pathFileName}palette.png -filter_complex \"fps=10,scale=160:-1:flags=lanczos[x];[x][1:v]paletteuse\" {$destinationFile}";');
            exec($ffmpeg . " < /dev/null 2>&1", $output, $return_val);
            if ($return_val !== 0) {
