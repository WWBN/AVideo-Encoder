...
// Security: validate URL and sanitize for shell usage
...
if (preg_match('/(youtube.com|youtu.be|vimeo.com|rumble.com)/', $url)) {
    ...
    $destination .= ".".$_GET['format'];
    //Generate a palette:
    $ffmpegPallet =get_ffmpeg()." -y -t 3 -i {$urlEscaped} -vf fps=10,".getFFmpegScaleToForceOriginalAspectRatio(320, 180).":flags=lanczos,palettegen {$destinationPallet}";
    $ffmpegPallet = removeUserAgentIfNotURL($ffmpegPallet);
    $exec =get_ffmpeg()." -y -t 3 -i {$urlEscaped} -i {$destinationPallet} -filter_complex \"fps=10,".getFFmpegScaleToForceOriginalAspectRatio(320, 180).":flags=lanczos[x];[x][1:v]paletteuse\" {$destination}";
    ...
    $destination .= ".".$_GET['format'];
    $exec =get_ffmpeg()." -y -ss 3 -t 3 -i {$urlEscaped} -vcodec libwebp -lossless 1 -vf fps=10,".getFFmpegScaleToForceOriginalAspectRatio(640, 360)." -q 60 -preset default -loop 0 -an -vsync 0 {$destination}";
    $destinationTmpFile = "{$global['systemRootPath']}view/img/notfound.gif";
} else {
...
if (!file_exists($destination) || fileOlderThen($destination, $cache_life) || !empty($_GET['renew'])) {
    if (!empty($ffmpegPallet)) {
        ...
        if (is_readable($destinationPallet)) {
            ...
            $cmdGif = get_ffmpeg()."  -y -t 3 -i {$urlEscaped} -vf fps=10,".getFFmpegScaleToForceOriginalAspectRatio(320, 180)." {$destination}";
