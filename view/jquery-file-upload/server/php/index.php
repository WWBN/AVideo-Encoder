...
if (isset($_FILES['files']) && $_FILES['files']['error'] == 0) {
    ...
    $obj->files = $response['files'];
    $extension = pathinfo($_FILES['files']['name'], PATHINFO_EXTENSION);

    if (!in_array(strtolower($extension), $global['allowed'])) {
        $obj->msg = "File extension error (" . $_FILES['files']['name'] . "), we allow only (" . implode(",", $global['allowed']) . ")";
        die(json_encode($obj));
    }
    ...
    $_FILES['files']['tmp_name'] = "{$global['systemRootPath']}videos/".$obj->files[0]->name;
    
    if (!rename($_FILES['files']['tmp_name'], $destinationFile)) {
        $obj->msg = "Error on move_uploaded_file(" . $_FILES['files']['tmp_name'] . ", " . "{$global['systemRootPath']}videos/original_" . $filename . ")";
        die(json_encode($obj));
    }
    ...
    error_log("Encoder jQuery-File-Upload: ".json_encode($obj));
}
